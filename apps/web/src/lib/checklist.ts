...
// How a Documents item behaves when opened from the grid:
//   upload  — user uploads an external file (PDF, image)
//   form    — user fills out fields inside the modal (no upload)
//   summary — read-only summary of data we already know (payments, calendar)
export type ChecklistItemKind = "upload" | "form" | "summary";
...
type TemplateItem = {
  ...
  sort_order: number;
  url?: string;
  kind?: ChecklistItemKind; // Only defined for Documents items that open a modal
};

export const CHECKLIST_TEMPLATE: TemplateItem[] = [
  // Documents (10)
  { category: "documents", item_key: "host_rental_agreement", label: "Host Rental Agreement", sort_order: 1, kind: "form" },
  { category: "documents", item_key: "paid_onboarding_fee", label: "Paid Initial Onboarding Fee", sort_order: 2, kind: "summary" },
  { category: "documents", item_key: "ach_authorization", label: "ACH Authorization Form", sort_order: 3, kind: "form" },
  { category: "documents", item_key: "card_authorization", label: "Card Authorization Form", sort_order: 4, kind: "form" },
  { category: "documents", item_key: "w9_form", label: "W9 Form", sort_order: 5, kind: "upload" },
  { category: "documents", item_key: "identity_verification", label: "Identity Verification", sort_order: 6, kind: "upload" },
  { category: "documents", item_key: "property_setup_form", label: "Property Setup Form", sort_order: 7, kind: "form" },
  { category: "documents", item_key: "wifi_info", label: "Wi-Fi Account Information", sort_order: 8, kind: "form" },
  { category: "documents", item_key: "guidebook_recommendations", label: "Recommendations for Guidebook", sort_order: 9, kind: "form" },
  { category: "documents", item_key: "block_dates_calendar", label: "Block Dates on the Calendar", sort_order: 10, kind: "summary" },
...
/* ─── Modal kind lookup by item_key (Documents only) ─── */

const KIND_MAP = new Map(
  CHECKLIST_TEMPLATE.filter((t) => t.kind).map((t) => [t.item_key, t.kind!]),
);

export function getItemKind(itemKey: string): ChecklistItemKind | undefined {
  return KIND_MAP.get(itemKey);
}

...
export type EntityContactProperty = {
  id: string;
  label: string;
...
export type EntityContactDetail = {
  ...
  properties: EntityContactProperty[];
  lifetimeRevenue: number | null;
};

type EntityContactPropertyQueryRow = {
  id: string;
  name: string | null;
...
export async function fetchEntityContactDetail(contactId: string): Promise<EntityContactDetail | null> {
  ...
  if (profileId) {
    ...
    if (profile?.entity_id) {
      ...
      // junction table, then deduplicate by ID — same pattern as entity-detail.ts.
      ...
      const { data: props } = propertyIds.length > 0
        ? await db
            .from<EntityContactPropertyQueryRow[]>("properties")
            .select(
              "id, name, address_line1, city, state, setup_status, active, bedrooms, bathrooms, created_at"
            )
            .in("id", propertyIds)
        : { data: [] as EntityContactPropertyQueryRow[] };
...
export async function fetchEntityMembers(entityId: string): Promise<EntityMember[]> {
  ...
  const { data, error } = await db
    .from<EntityMemberContactRow[]>("contacts")

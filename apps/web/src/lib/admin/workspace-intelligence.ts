...
const SYSTEM_PROMPT = `You are a property management CRM assistant analyzing an workspace relationship for Johan, a short-term rental property manager. You will receive context about an workspace and the people attached to it, including contact history, meetings, and communications, then return a structured JSON analysis.
...
Return ONLY valid JSON matching this exact schema:
{
  ...
  "recommendations": [
    { "title": "string (max 60 chars)", "body": "string (1-2 sentences, specific and actionable)" }
  ],
  "conversation_themes": [
...
recommendations: exactly 2-3 items. Specific to this workspace relationship, not generic advice.
...
async function callClaude(userPrompt: string): Promise<IntelligenceResult | null> {
  ...
  if (!apiKey) {
    console.error("[workspace-intelligence] OPENROUTER_API_KEY not set");
    return null;
  }
  ...
  if (!res.ok) {
    console.error("[workspace-intelligence] Claude call failed:", await res.text());
    return null;
  }
  ...
  try {
    ...
    console.error("[workspace-intelligence] JSON parse failed:", raw.slice(0, 200));
    return null;
  }
}
...
function buildPrompt(context: {
  ...
  const parts: string[] = [
    `Workspace contact: ${context.contactName}`,
...
export async function generateWorkspaceIntelligence(contactId: string): Promise<void> {
  ...
  if (!contact) {
    console.error("[workspace-intelligence] contact not found:", contactId);
    return;
  }
  ...
  // Delete old relationship intelligence insights before inserting fresh ones.
  ...
  if (deleteError) {
    console.error("[workspace-intelligence] delete error:", deleteError.message);
    return;
  }

  const { error } = await serviceClient.from("ai_insights").insert(rows);
  if (error) {
    console.error("[workspace-intelligence] insert error:", error.message);
  }
}

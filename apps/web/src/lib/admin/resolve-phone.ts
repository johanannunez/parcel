...
import type { ResolvedCommunicationTarget } from './communication-types';

/**
 * Resolve a phone number to a relationship target in priority order:
 ...
 */
export async function resolvePhone(rawPhone: string): Promise<ResolvedCommunicationTarget> {
  ...
  if (contactResult.data) {
    return {
      type: 'contact',
      targetId: contactResult.data.id,
      displayName:

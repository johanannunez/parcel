...
const SYSTEM_PROMPT = `You are a property management CRM assistant analyzing an entity relationship for Johan, a short-term rental property manager. You will receive context about an entity and the people attached to it, including contact history, meetings, and communications, then return a structured JSON analysis.
...
recommendations: exactly 2-3 items. Specific to this entity relationship, not generic advice.
...
async function callClaude(userPrompt: string): Promise<IntelligenceResult | null> {
  ...
  if (!apiKey) {
    console.error("[entity-intelligence] OPENROUTER_API_KEY not set");
    return null;
  }
  ...
  if (!res.ok) {
    console.error("[entity-intelligence] Claude call failed:", await res.text());
    return null;
  }
  ...
  try {
    ...
    console.error("[entity-intelligence] JSON parse failed:", raw.slice(0, 200));
    return null;
  }
}
...
function buildPrompt(context: {
  ...
  const parts: string[] = [
    `Entity contact: ${context.contactName}`,
...
export async function generateEntityIntelligence(contactId: string): Promise<void> {
  ...
  if (!contact) {
    console.error("[entity-intelligence] contact not found:", contactId);
    return;
  }
  ...
  if (deleteError) {
    console.error("[entity-intelligence] delete error:", deleteError.message);
    return;
  }

  const { error } = await serviceClient.from("ai_insights").insert(rows);
  if (error) {
    console.error("[entity-intelligence] insert error:", error.message);
  }
}

...
const PEOPLE_LIST_LOADED_AT_MS = Date.now();
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (localFilters.activityWindow) {
        ...
        const lastMs = r.lastActivityAt ? PEOPLE_LIST_LOADED_AT_MS - new Date(r.lastActivityAt).getTime() : Infinity;
      ...
      <div className={styles.tableWrap}>
        ...
        {filtered.length === 0 ? (
          <div className={styles.empty}>
            No people match this view{search ? ` with "${search}"` : ''}.
          </div>
        ) : null}
      </div>
    </div>

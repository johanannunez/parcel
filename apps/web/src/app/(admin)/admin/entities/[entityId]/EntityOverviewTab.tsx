...
import type { EntityContactDetail } from "@/lib/admin/entity-contact-detail";
import type { EntityDocument } from "@/lib/admin/entity-documents";
import type { EntityMessage } from "@/lib/admin/entity-messages";
import type { Insight } from "@/lib/admin/ai-insights";
import type { OverviewTask } from "@/lib/admin/entity-overview";
import type { EntityDetailActivityEntry } from "@/lib/admin/entity-detail-types";
import { regenerateEntityIntelligence } from "./entity-intelligence-actions";
...
type Props = {
  ...
  activityLog: EntityDetailActivityEntry[];
};
...
export function EntityOverviewTab({
  ...
  if (unreadMessages.length > 0) {
    attnItems.push({
      id: "unread-msgs",
      level: "amber",
      title: `${unreadMessages.length} unread message${unreadMessages.length > 1 ? "s" : ""} from ${entityContact.fullName.split(" ")[0]}`,
      sub: "Needs a reply",
      cta: "Communication →",
      href: `?tab=messaging`,
    });
  ...
  if (entityContact.preferredContactMethod) {
    const method =
      entityContact.preferredContactMethod === "phone"
        ? "📞 Prefers phone"
        : entityContact.preferredContactMethod === "text"
          ? "💬 Prefers text"
          : entityContact.preferredContactMethod === "whatsapp"
            ? "💬 Prefers WhatsApp"
            : "📧 Prefers email";
  ...
  function handleRefresh() {
    startTransition(async () => {
      await regenerateEntityIntelligence(entityContact.id);
      router.refresh();
    });
  }
  ...
  const contactPath = `/admin/entities/${entityContact.id}`;
    ...
    <div className={styles.root}>
        ...
        <div className={styles.card}>
          ...
          <div className={styles.statBody}>
            <div className={styles.bigNum}>
              {entityContact.lifetimeRevenue != null
                ? entityContact.lifetimeRevenue >= 100_000
                  ? `$${(entityContact.lifetimeRevenue / 100).toFixed(0)}`
                  : `$${(entityContact.lifetimeRevenue / 100).toFixed(0)}`
                : "$0"}
            </div>
            <div className={styles.bigSub}>lifetime revenue</div>
            <div className={styles.statList}>
              {entityContact.managementFeePercent != null && (
                <div className={styles.statRow}>
                  <span className={`${styles.sDot} ${styles.green}`} />
                  <span>{entityContact.managementFeePercent}% management fee</span>
                </div>
              )}
              <div className={styles.statRow}>
                <span className={`${styles.sDot} ${styles.green}`} />
      ...
      <div className={styles.bento}>
        ...
        <div className={styles.card}>
          ...
          {entityContact.properties.length === 0 ? (
            <div className={styles.emptyHint}>No properties yet</div>
          ) : (
            entityContact.properties.map((p) => (

...
import type { EntityContactDetail, AddressComponents, EntityInfo, EntityMember, SocialLinks } from "@/lib/admin/entity-contact-detail";
import { useRouter, useSearchParams } from "next/navigation";
import type { AdminProfile } from "./entity-person-actions";
import { updateEntityContactFields, updateEmailWithPortalSync } from "./entity-person-actions";
import { DatePickerInput } from "@/components/admin/DatePickerInput";
import styles from "./EntityDetailSidebar.module.css";
...
export function EntityDetailSidebar({
  ...
  const [firstName,   setFirstName]   = useState(entityContact.firstName   ?? "");
  const [lastName,    setLastName]     = useState(entityContact.lastName    ?? "");
  const [email,       setEmail]        = useState(entityContact.email       ?? "");
  const [emailVerified, setEmailVerified] = useState(entityContact.emailVerified);
  const [phone,       setPhone]        = useState(entityContact.phone       ?? "");
  const source = entityContact.source ?? "";
  const [addressFmt,  setAddressFmt]   = useState(entityContact.addressFormatted ?? null);
  const [social,      setSocial]       = useState<SocialLinks>(entityContact.social ?? {});
  const [assignedTo,  setAssignedTo]   = useState(entityContact.assignedTo);
  const [contactMethod, setContactMethod] = useState(entityContact.preferredContactMethod);
  const [contractStart, setContractStart] = useState(entityContact.contractStartAt);
  const [contractEnd,   setContractEnd]   = useState(entityContact.contractEndAt);
  const [feePercent,    setFeePercent]    = useState(
    entityContact.managementFeePercent !== null ? String(entityContact.managementFeePercent) : ""
  );
  ...
  async function save(fields: Parameters<typeof updateEntityContactFields>[1]) {
    await updateEntityContactFields(entityContact.id, fields);
  }
  ...
  const saveEmailWithPortal = async (val: string): Promise<{ ok: boolean; message: string }> => {
    const result = await updateEmailWithPortalSync(entityContact.id, val);
    if (result.ok) {
      setEmail(val);
  ...
  const hasPortal = entityContact.profileId !== null;
  ...
  return (
    <aside className={styles.sidebar}>
      ...
      <EmailField
        ...
        profileId={entityContact.profileId}
      ...
      <AddressField
        value={addressFmt}
        components={entityContact.addressComponents}
        onSave={saveAddress}

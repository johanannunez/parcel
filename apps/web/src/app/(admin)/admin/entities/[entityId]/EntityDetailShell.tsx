...
import type { EntityContactDetail, EntityInfo, EntityMember } from "@/lib/admin/entity-contact-detail";
import type { NextMeeting } from "@/lib/admin/entity-meetings";
import type { AdminProfile } from "./entity-person-actions";
import { updateEntityContactFields } from "./entity-person-actions";
...
import { EntityNameContext } from "./EntityNameContext";
...
function StatusLine({
  ...
  const save = (iso: string | null) => {
    ...
    startTransition(async () => {
      await updateEntityContactFields(contactId, { nextFollowUpAt: iso });
    });
  };
...
function EntityContactDetailContent({
  entityContact,
  adminProfiles,
  nextMeeting,
  entityInfo,
  ...
  entityContact: EntityContactDetail;
  ...
  useSetTopBarSlots(
    () => ({ breadcrumbTrail: ["Entities", entityInfo.name, tabLabel] }),
  ...
  const [displayFirst, setDisplayFirst] = useState(entityContact.firstName ?? "");
  const [displayLast, setDisplayLast] = useState(entityContact.lastName ?? "");
  const [editingNamePart, setEditingNamePart] = useState<"first" | "last" | null>(null);
  const [displayAvatarUrl, setDisplayAvatarUrl] = useState<string | null>(entityContact.avatarUrl ?? null);
  ...
  return (
    <EntityNameContext.Provider value={{ firstName: displayFirst, lastName: displayLast, avatarUrl: displayAvatarUrl, setAvatarUrl: setDisplayAvatarUrl }}>
    <div ref={shellRef} className={styles.shell}>
      <div className={styles.leftColumn}>
        ...
        <header className={styles.header}>
          ...
          <div className={styles.headerGrid}>
            ...
            <div className={styles.contactBlock}>
              {displayAvatarUrl ? (
                <img src={displayAvatarUrl} alt={entityContact.fullName} className={styles.avatar} />
              ) : (
                <div className={styles.avatarFallback}>
                  {getInitials((displayFirst || displayLast) ? `${displayFirst} ${displayLast}`.trim() : entityContact.fullName)}
                </div>
              )}
              <div className={styles.identityBlock}>
                ...
                <h1 className={styles.name}>
                  {(displayFirst || displayLast) ? (
                    ...
                    entityContact.fullName || "—"
                  )}
                </h1>
                <div className={styles.contactStack}>
                  {entityContact.email && (
                    <span className={styles.contactItem}>
                      <span className={styles.contactValue}>{entityContact.email}</span>
                      <CopyButton value={entityContact.email} />
                    </span>
                  )}
                  {entityContact.phone && (
                    <span className={styles.contactItem}>
                      <span className={styles.contactValue}>{formatPhone(entityContact.phone) ?? entityContact.phone}</span>
                      <CopyButton value={entityContact.phone} />
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className={styles.pillsRow}>
              <div className={styles.pill}>
                <span className={styles.pillHighlight}>{entityContact.properties.length}</span>
                <span>{entityContact.properties.length === 1 ? "Property" : "Properties"}</span>
              </div>
              <div className={styles.pillSep} />
              <div className={styles.pill}>
                <span className={styles.pillHighlight}>{formatRevenue(entityContact.lifetimeRevenue)}</span>
                <span>Lifetime rev.</span>
              </div>
              <div className={styles.pillSep} />
              <StagePopover contactId={entityContact.id} stage={entityContact.lifecycleStage} />
            </div>

            <StatusLine
              contactId={entityContact.id}
              followUpAt={entityContact.nextFollowUpAt}
              nextMeeting={nextMeeting}
              lastActivityAt={entityContact.lastActivityAt}
            />
        ...
        <div className={styles.tabColumn}>
          <nav className={styles.tabBar} aria-label="Entity sections">
            {TABS.map((tab) => (
              <button
                key={tab.key}
                type="button"

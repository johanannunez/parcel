...
import { useEntityName } from "@/app/(admin)/admin/entities/[entityId]/EntityNameContext";
...
export function PersonalInfoSection({
  ...
  const [firstName, setFirstName] = useState(entityName?.firstName ?? seeded.first);
  const [lastName, setLastName] = useState(entityName?.lastName ?? seeded.last);

  // Live sync from the sidebar when inside the entity detail shell.
  const contextFirst = entityName?.firstName;
  const contextLast = entityName?.lastName;
  useEffect(() => {
  ...
  const handleCrop = async (croppedDataUrl: string) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        entityName?.setAvatarUrl(result.avatarUrl);
      }
    } catch {

...
import type { AddressComponents, SocialLinks } from "@/lib/admin/entity-contact-detail";
...
// ---------------------------------------------------------------------------
// Update entity person fields
// ---------------------------------------------------------------------------

type UpdateEntityContactFields = Partial<{
  firstName: string;
  lastName: string;
  email: string;
  phone: string;
...
export async function updateEntityContactFields(
  contactId: string,
  fields: UpdateEntityContactFields
...
export async function updateEntityFields(
  ...
  const { error } = await untypedDatabase(supabase).from("entities").update(updates).eq("id", entityId);

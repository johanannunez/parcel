...
import { sendEntityMessage, togglePinMessage } from "./messaging-actions";
...
export function MessagingTab({ messages: initialMessages, members, activeContactId }: Props) {
  const [localMessages, setLocalMessages] = useState<EntityMessage[]>(initialMessages);
  ...
  function handleSend() {
    ...
    startTransition(async () => {
      const result = await sendEntityMessage(composeContactId, body);
      if (result.ok) {
        setBody("");
        ...
        {thread.length === 0 && pinned.length === 0 ? (
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyBody}>
              Messages sent here are private notes and in-app communications with this entity.
            </p>
          </div>
        ) : (
          thread.map((m) => {

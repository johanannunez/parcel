...
export default async function EntitiesLayout({ children }: { children: ReactNode }) {
  ...
  const views = allViews.filter((v) => {
    if (!v.isPersonal) return ENTITY_VIEW_KEYS.includes(v.key);
    const baseView = v.searchParams?.view;
    return baseView ? ENTITY_VIEW_KEYS.includes(baseView) : true;
  });

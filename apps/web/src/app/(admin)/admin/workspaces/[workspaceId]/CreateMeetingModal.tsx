...
import type { AdminProfile } from "@/app/(admin)/admin/workspaces/[workspaceId]/workspace-person-actions";
...
const TIME_OPTIONS = Array.from({ length: 32 }, (_, index) => {
  ...
  const label = new Date(2026, 0, 1, hours, minutes).toLocaleTimeString(
    "en-US",
    {
      hour: "numeric",
      minute: "2-digit",
...
export function CreateMeetingModal({
  ...
  function handleTypeSelect(type: MeetingType) {
    ...
    setTitle(TYPE_CONFIG[type].defaultTitle(ownerFirstName));
  }
          ...
          <div className={styles.row3}>
            <div className={styles.field}>
              ...
              <DatePickerInput
                value={date}
                onChange={setDate}
                className={[styles.input, styles.dateButton].join(" ")}
              />
            </div>
            <div className={styles.field}>
              <label className={styles.label}>
                  ...
                  return (
                    ...
                    >
                      <div className={[styles.attendeeAvatar, selected ? styles.attendeeAvatarActive : ""].join(" ")}>
                        {p.avatarUrl
                          ? <Image src={p.avatarUrl} alt={p.fullName} width={28} height={28} className={styles.avatarImg} />
                          : initials(p.fullName)
                        }
                      </div>

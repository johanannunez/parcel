...
const PROSPECT_VIEW_KEYS = ['lead-pipeline', 'onboarding', 'archived'];
...
export default async function ProspectsLayout({ children }: { children: ReactNode }) {
  ...
  const views = allViews.filter((v) => {
    if (!v.isPersonal) return PROSPECT_VIEW_KEYS.includes(v.key);
    const baseView = v.searchParams?.view;
    return baseView ? PROSPECT_VIEW_KEYS.includes(baseView) : true;
  });

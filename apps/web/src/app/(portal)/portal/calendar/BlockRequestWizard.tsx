      ...
      <div>
        ...
        <div className="grid grid-cols-7">
          {calDays.map((iso, i) => {
            ...
            return (
              <button key={iso} type="button" disabled={isPast} onClick={() => onDayClick(iso)} onMouseEnter={() => { if (data.startDate && !data.endDate) setHovered(iso); }} onMouseLeave={() => setHovered(null)} className="relative flex h-8 items-center justify-center text-[12px] font-medium tabular-nums transition-colors" style={{ color: isPast ? "var(--color-text-tertiary)" : (start || end) ? "var(--color-white)" : inRange ? "var(--color-brand)" : "var(--color-text-primary)", backgroundColor: (start || end) ? "var(--color-brand)" : inRange ? "rgba(2, 170, 235, 0.14)" : "transparent", borderRadius: start && end ? "6px" : start ? "6px 0 0 6px" : end ? "0 6px 6px 0" : "0", cursor: isPast ? "default" : "pointer", opacity: isPast ? 0.4 : 1, fontWeight: isToday || start || end ? 700 : 500 }}>
